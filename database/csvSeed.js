...
var headerListing = 'shareId,name,rating,reviews,location\n';
...
const fakeListings = function(start) {
  ...
  while (count <= totalRecords) {
    ...
    var location = faker.address.city() + ', ' + faker.address.state() + ', ' + faker.address.country();
...
const fakePhotos = function(start) {
  ...
  while (count <= totalRecords) {
    ...
    for (var j = 0; j < randomPhotos; j++) {
      ...
      if (!writeStreamPhotos.write(`${shareId}, ${photoList[randomIndex]}\n`)) {
